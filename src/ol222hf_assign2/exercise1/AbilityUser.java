...
public abstract class AbilityUser extends Character{
     ...
     */
    public AbilityUser(String name, int level, ClassName className, Attributes attributes, Set<Proficiency> proficiencies) {
     ...
     * @throws IllegalArgumentException when the ability user's and the ability's proficiencies does not match.
     */
    public void learnAbility(Ability ability) throws IllegalArgumentException {
        if (proficiencies.contains(ability.getProficiency())) {
            ...
            throw new IllegalArgumentException(ability + " with proficiency " + ability.getProficiency() + " is an invalid ability for this character.");
        }
    }
