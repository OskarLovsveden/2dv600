...
/**
 * This class defines methods that are related to depth-first traversals. They
 * can be applied on all graphs of type DirectedGraph having nodes extending the
 * class Node.
 * <p/>
 * The dfs methods returns a list (java.util.List) of nodes in depth-first
 * order. They also annotates each node (the public field Node.num) with a
 * depth-first number.
 * <p/>
 * The postOrder methods returns a list (java.util.List) of nodes in post-order
 * of the depth first tree. They also annotates each node (the public field
