...
*/
package ol222hf_assign3.graphs;

import java.util.*;

/**
 * An abstract <tt>Node</tt> class that serves as an interface for nodes of the
 * <tt>DirectedGraph</tt> interface. Each node is aware of (and stores) all its
 * successor and predecessor nodes.
 * </p>
 * 
 * This class also defines a set of protected methods that can be used to add
 * and remove adjacent nodes. They should only be used by implementors of the
 * <tt>DirectedGraph</tt> interface. Reason: improper use of these methods may
 * put the graph in an inconsistent state. For example, by removing A as
 * successor of B without removing B as predecessor of A.
 * </p>
 * 
 * The public integer field <tt>num</tt> is introduced to avoid externally
 * constructed mappings between the node and some integer (e.g. a dfs number).
 * It should only be used internally and never be a part of any public interface
 * since its interpretation might be changed from one algorithm to another.
 * 
 * @author jonasl
 *
 */
public abstract class Node<E> {
	 ...
	 * 
	 * @param node a possible successor node
	 * @return boolean
	 */
	 ...
	 * otherwise <tt>false</tt>.
	 * 
	 * @param node a possible predecessor node
	 * @return boolean
	 */
	...
	/**
	 * Returns true if <tt>this</tt> node has a reflexive edge (i.e. an out-edge
	 * targeting itself).
	 */
	...
	/**
	 * Removes reflexive edge if it exists. (i.e. an out-edge targeting itself).
	 */
	 ...
	 * ====================================================================
	 * 
	 * Internal methods intended to be hidden for all clients (graph users). They
	 * should only be used by implementers of the <tt>DirectedGraph</tt> interface.
	 * Reason: improper use of these methods may put the graph in an inconsistent
	 * state.
	 * 
	 * ====================================================================
	 */
